...
function collapseFilament(element: IFilament): IFilamentCollapsed {
    ...
    let preset_name: string | null;
    if (element.preset) {
        preset_name = element.preset.name;
    } else {
        preset_name = null;
    }

    return { ...element, "vendor.name": vendor_name, "preset.name": preset_name };
}
...
const defaultColumns = allColumns.filter(
    (column_id) =>
        ["registered", "density", "diameter", "spool_weight", "article_number", "settings_extruder_temp", "settings_bed_temp"].indexOf(
            column_id
        ) === -1
);
            ...
            <Table<IFilamentCollapsed>
                ...
                columns={removeUndefined([
                    ...
                    SortedColumn({
                        ...commonProps,
                        id: "preset.name",
                        i18nkey: "filament.fields.preset",
                    }),
                    SpoolIconColumn({
                        ...commonProps,
                        id: "name",
