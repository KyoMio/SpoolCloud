...
from spoolcloud.api.v1.models import EventType, Vendor, VendorEvent
from spoolcloud.database import models
from spoolcloud.database.utils import SortOrder, add_where_clause_str, add_where_clause_str_opt
...
async def find(
    *,
    ...
    """
    stmt = select(models.Vendor).where(models.Vendor.user_id == user_id)
...
async def update(
    *,
    ...
    vendor = await get_by_id(db, vendor_id, user_id)
...
async def delete(db: AsyncSession, vendor_id: int, user_id: int) -> None:
