...
from spoolcloud.prometheus.metrics import filament_metrics, spool_metrics
...
def get_connection_url() -> URL:
    ...
    if db_type is None:
        ...
        database = str(env.get_data_dir().joinpath("spoolcloud.db"))
...
class Database:
    ...
    def backup_and_rotate(
        ...
        backup_path = backup_folder.joinpath(f"spoolcloud.db.{num_backups}")
        ...
        for i in range(num_backups - 1, -1, -1):
            if i == 0:
                backup_path = backup_folder.joinpath("spoolcloud.db")
            else:
                backup_path = backup_folder.joinpath(f"spoolcloud.db.{i}")
            if backup_path.exists():
                logger.debug("Rotating backup %s to %s", backup_path, backup_folder.joinpath(f"spoolcloud.db.{i + 1}"))
                shutil.move(backup_path, backup_folder.joinpath(f"spoolcloud.db.{i + 1}"))
