...
from spoolcloud.math import delta_e, hex_to_rgb, rgb_to_lab
...
async def create(
    *,
    ...
    if vendor_id is not None:
        vendor_item = await vendor.get_by_id(db, vendor_id, user_id)
...
async def get_by_id(db: AsyncSession, filament_id: int, user_id: int) -> models.Filament:
...
async def update(
    *,
    ...
    filament = await get_by_id(db, filament_id, user_id)
    for k, v in data.items():
        if k == "vendor_id":
            if v is None:
                filament.vendor = None
            else:
                filament.vendor = await vendor.get_by_id(db, v, user_id)
        elif k == "extra":
...
async def delete(db: AsyncSession, filament_id: int, user_id: int) -> None:
...
async def find_materials(
    *,
    ...
    stmt = select(models.Filament.material).where(models.Filament.user_id == user_id).distinct()
...
async def find_article_numbers(
    *,
    ...
    stmt = select(models.Filament.article_number).where(models.Filament.user_id == user_id).distinct()
    rows = await db.execute(stmt)
    return [row[0] for row in rows.all() if row[0] is not None]
...
async def find_by_color(
    *,
    ...
    """
    filaments, _ = await find(db=db, user_id=user_id)
