...
from spoolcloud.api.v1.models import EventType, Spool, SpoolEvent
from spoolcloud.database import filament, models
from spoolcloud.database.utils import (
    SortOrder,
...
from spoolcloud.exceptions import ItemCreateError, ItemNotFoundError, SpoolMeasureError
from spoolcloud.math import weight_from_length
...
async def create(
    *,
    ...
    filament_item = await filament.get_by_id(db, filament_id, user_id)
...
async def update(
    *,
    ...
    spool = await get_by_id(db, spool_id, user_id)
    for k, v in data.items():
        if k == "filament_id":
            spool.filament = await filament.get_by_id(db, v, user_id)
...
async def delete(db: AsyncSession, spool_id: int, user_id: int) -> None:
...
async def use_weight_safe(db: AsyncSession, spool_id: int, user_id: int, weight: float) -> None:
    ...
    await db.execute(
        ...
        .where(models.Spool.id == spool_id, models.Spool.user_id == user_id)
        .values(
            used_weight=case(
...
async def use_weight(db: AsyncSession, spool_id: int, user_id: int, weight: float) -> models.Spool:
    ...
    """
    await use_weight_safe(db, spool_id, user_id, weight)
...
async def use_length(db: AsyncSession, spool_id: int, user_id: int, length: float) -> models.Spool:
...
async def measure(db: AsyncSession, spool_id: int, user_id: int, weight: float) -> models.Spool:
    ...
    if weight > initial_gross_weight:
        return await reset_initial_weight(db, spool_id, user_id, weight - spool_weight)
    ...
    return await use_weight(db, spool_id, user_id, weight_to_use)
...
async def find_locations(
    *,
    ...
    stmt = sqlalchemy.select(models.Spool.location).where(models.Spool.user_id == user_id).distinct()
...
async def find_lot_numbers(
    *,
    ...
    stmt = sqlalchemy.select(models.Spool.lot_nr).where(models.Spool.user_id == user_id).distinct()
    rows = await db.execute(stmt)
    return [row[0] for row in rows.all() if row[0] is not None]
...
async def reset_initial_weight(db: AsyncSession, spool_id: int, user_id: int, weight: float) -> models.Spool:
...
async def rename_location(
    *,
    ...
    await db.execute(
        sqlalchemy.update(models.Spool)
        .where(models.Spool.location == current_name, models.Spool.user_id == user_id)
