...
def get_database_type() -> Optional[DatabaseType]:
    ...
    """
    database_type = os.getenv("SPOOLCLOUD_DB_TYPE")
    ...
    raise ValueError(f"Failed to parse SPOOLCLOUD_DB_TYPE variable: Unknown database type '{database_type}'.")
...
def get_host() -> Optional[str]:
    ...
    """
    return os.getenv("SPOOLCLOUD_DB_HOST")
...
def get_port() -> Optional[int]:
    ...
    """
    port = os.getenv("SPOOLCLOUD_DB_PORT")
    if port is None:
        return None
    try:
        return int(port)
    except ValueError as exc:
        raise ValueError(f"Failed to parse SPOOLCLOUD_DB_PORT variable: {exc!s}") from exc
...
def get_database() -> Optional[str]:
    ...
    """
    return os.getenv("SPOOLCLOUD_DB_NAME")
...
def get_query() -> Optional[dict[str, str]]:
    ...
    """
    query = os.getenv("SPOOLCLOUD_DB_QUERY")
    if query is None:
        return None
    try:
        ...
        raise ValueError(f"Failed to parse SPOOLCLOUD_DB_QUERY variable: {exc!s}") from exc
...
def get_username() -> Optional[str]:
    ...
    """
    return os.getenv("SPOOLCLOUD_DB_USERNAME")
...
def get_password() -> Optional[str]:
    ...
    file_path = os.getenv("SPOOLCLOUD_DB_PASSWORD_FILE")
    if file_path is not None:
        file = Path(file_path)
        if not file.exists() or not file.is_file():
            raise ValueError(
                "Failed to parse SPOOLCLOUD_DB_PASSWORD_FILE variable: "
    ...
    return os.getenv("SPOOLCLOUD_DB_PASSWORD")
...
def get_logging_level() -> int:
    ...
    """
    log_level_str = os.getenv("SPOOLCLOUD_LOGGING_LEVEL", "INFO").upper()
    ...
    raise ValueError(f"Failed to parse SPOOLCLOUD_LOGGING_LEVEL variable: Unknown logging level '{log_level_str}'.")
...
def is_debug_mode() -> bool:
    ...
    """
    debug_mode = os.getenv("SPOOLCLOUD_DEBUG_MODE", "FALSE").upper()
    if debug_mode in {"FALSE", "0"}:
        return False
    if debug_mode in {"TRUE", "1"}:
        return True
    raise ValueError(f"Failed to parse SPOOLCLOUD_DEBUG_MODE variable: Unknown debug mode '{debug_mode}'.")
...
def is_cors_defined() -> bool:
    ...
    """
    cors = os.getenv("SPOOLCLOUD_CORS_ORIGIN", "FALSE").upper()
...
def get_cors_origin() -> Optional[list[str]]:
    ...
    """
    cors = os.getenv("SPOOLCLOUD_CORS_ORIGIN")
    if cors is None:
        return None
    return cors.split(",")
...
def is_automatic_backup_enabled() -> bool:
    ...
    """
    automatic_backup = os.getenv("SPOOLCLOUD_AUTOMATIC_BACKUP", "TRUE").upper()
    if automatic_backup in {"FALSE", "0"}:
        return False
    if automatic_backup in {"TRUE", "1"}:
        return True
    raise ValueError(
        f"Failed to parse SPOOLCLOUD_AUTOMATIC_BACKUP variable: Unknown automatic backup '{automatic_backup}'.",
    )
...
def get_data_dir() -> Path:
    ...
    """
    env_data_dir = os.getenv("SPOOLCLOUD_DIR_DATA")
    if env_data_dir is not None:
        data_dir = Path(env_data_dir)
    else:
        data_dir = Path(user_data_dir("spoolcloud"))
...
def get_logs_dir() -> Path:
    ...
    """
    env_logs_dir = os.getenv("SPOOLCLOUD_DIR_LOGS")
...
def get_backups_dir() -> Path:
    ...
    """
    env_backups_dir = os.getenv("SPOOLCLOUD_DIR_BACKUPS")
        ...
        if not chown_dir(str(get_data_dir())) or not can_write_to_data_dir():
            ...
            logger.error(
                (
                    ...
                    'Please run "sudo chown -R %s:%s /path/to/spoolcloud/datadir" on the host OS.'
                ),
                uid,
                gid,
            )
...
def is_metrics_enabled() -> bool:
    ...
    """
    metrics_enabled = os.getenv("SPOOLCLOUD_METRICS_ENABLED", "FALSE").upper()
    if metrics_enabled in {"FALSE", "0"}:
        return False
    if metrics_enabled in {"TRUE", "1"}:
        return True
    raise ValueError(
        f"Failed to parse SPOOLCLOUD_METRICS_ENABLED variable: Unknown metrics enabled '{metrics_enabled}'.",
    )
...
def get_base_path() -> str:
    ...
    """
    path = os.getenv("SPOOLCLOUD_BASE_PATH", "")
    if len(path) == 0:
        return ""
...
def get_auth_secret() -> str:
    ...
    secret = os.getenv("SPOOLCLOUD_AUTH_SECRET")
    if secret is None:
        logger.warning("SPOOLCLOUD_AUTH_SECRET is not set, using a default secret. THIS IS INSECURE!")
        return "insecure-default-secret-change-me"
    return secret


def is_registration_allowed() -> bool:
    """Get whether user registration is allowed from environment variables."""
    allow_registration = os.getenv("SPOOLCLOUD_ALLOW_REGISTRATION", "FALSE").upper()
    return allow_registration in {"TRUE", "1"}
